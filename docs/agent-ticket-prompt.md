...
4. For Ready, use at most one `agent:*` label. No selector means server default Codex. Prefer an explicit selector when the operator named a harness.
...
8. Success requires author `<trusted-github-login>`, state `OPEN`, exactly one supported `board:*` label, and no more than one supported `agent:*` label. If verification fails, do not add Ready. Report the mismatch.
